...
function BookModal({ test, isAr, onClose }: { test: typeof LABS[0]; isAr: boolean; onClose: () => void }) {
  ...
  const SLOT_GROUPS = [
    { key: "morning",   en: "Morning",   ar: "الصباح",
      slots: test.slots.filter(s => s.t.includes("AM")) },
    { key: "afternoon", en: "Afternoon", ar: "الظهيرة",
      slots: test.slots.filter(s => { const h = parseInt(s.t); return s.t.includes("PM") && (h === 12 || h <= 4); }) },
  ].filter(g => g.slots.length > 0);

  const STEPS = [
    { k: "date",    en: "Date",    ar: "التاريخ" },
    { k: "time",    en: "Time",    ar: "الوقت"   },
    { k: "payment", en: "Pay",     ar: "الدفع"   },
  ];
  ...
  const PAY_METHODS = [
    ...
    { key: "cash",    icon: "🏥", en: "Pay at Lab",          enSub: "Pay on arrival",    ar: "الدفع في المختبر",     arSub: "ادفع عند الوصول"    },
  ];
    ...
    <div className="px-6 pt-6 pb-4 border-b border-[#e7dcee] dark:border-[#2a1840]">
      ...
      <div className={`flex items-center gap-2 mt-4 ${isAr ? "flex-row-reverse" : ""}`}>
        {STEPS.map((s, i) => {
          ...
          return (
            <div key={s.k} className={`flex items-center gap-1.5 ${isAr ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`flex items-center gap-1.5 ${isAr ? "flex-row-reverse" : ""}`}>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black flex-shrink-0 transition-all ${
                  active ? "bg-[#2E1A47] dark:bg-[#DFC8E7] text-white dark:text-[#1a1030]"
                  : done  ? "bg-emerald-500 text-white"
                  :         "bg-[#e7dcee] dark:bg-[#3a2560] text-[#2E1A47]/40 dark:text-[#DFC8E7]/40"
                }`}>
                  {done ? "✓" : i + 1}
                </div>
                <span className={`text-xs font-semibold ${active ? "text-[#2E1A47] dark:text-[#DFC8E7]" : "text-[#2E1A47]/35 dark:text-[#DFC8E7]/35"}`}>
                  {isAr ? s.ar : s.en}
                </span>
              </div>
            </div>
  ...
  if (booked) return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white dark:bg-[#1a1030] rounded-2xl p-8 max-w-sm w-full text-center border border-[#e7dcee] dark:border-[#3a2560] shadow-2xl">
        ...
        <p className="text-xs text-[#2E1A47]/40 dark:text-[#DFC8E7]/40 mt-1">{info.lab}</p>
        {/* Payment summary */}
      ...
      <div className="bg-white dark:bg-[#1a1030] rounded-2xl max-w-md w-full border border-[#e7dcee] dark:border-[#3a2560] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        {step === "time" && (
          <div className="px-6 py-5">
            ...
            <button disabled={!selTime} onClick={() => setStep("payment")}
          ...
          <div className="px-6 py-5">
            ...
            <div className={`bg-[#faf8fc] dark:bg-[#0d0820] rounded-2xl p-4 mb-4 border border-[#e7dcee] dark:border-[#2a1840] space-y-2 ${isAr ? "text-right" : ""}`}>
              ...
              <div className={`flex justify-between items-center pt-2 border-t border-[#e7dcee] dark:border-[#2a1840] ${isAr ? "flex-row-reverse" : ""}`}>
                ...
                <span className="text-sm font-black text-[#46255f] dark:text-[#DFC8E7]">{isAr ? `${test.price} ر.ع.` : `OMR ${test.price}`}</span>
              </div>
            </div>
            ...
            <button disabled={!payMethod} onClick={() => setBooked(true)}
              ...
              {payMethod === "cash"
                ...
                : (isAr ? `ادفع ${test.price} ر.ع.` : `Pay OMR ${test.price}`)}
            </button>
          </div>
        )}
      </div>
