...
// NOTE two distinct specialty helpers coexist here on purpose:
//  • this module-scope `specialtyLabel(freetext, isAr)` localizes a doctor's FREETEXT
//    `doctors.specialty` for display (used by DoctorCard below);
//  • FindDoctorsPage defines a local `specialtyLabel(chip)` that localizes a curated
//    CATALOG chip for the filter pills. It shadows this one only inside that component.
    ...
    <div dir={ar ? "rtl" : "ltr"} className="min-h-screen bg-[#f9f4fa] dark:bg-[#0f0a1e] text-[#2E1A47] dark:text-[#DFC8E7]">
      ...
      <section className="bg-white dark:bg-[#0d0820] border-b border-[#e7dcee] dark:border-[#2a1840] px-4 py-4">
        <div className={`max-w-6xl mx-auto flex flex-wrap items-center gap-2 ${ar ? "flex-row-reverse" : ""}`}>
          ...
          {/* Specialty pills — dynamic backend catalog; wraps to a new line instead of clipping/scrolling */}
