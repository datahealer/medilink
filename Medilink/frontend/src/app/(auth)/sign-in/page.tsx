...
export default function SignInPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setError(ar ? "فشل تسجيل الدخول بـ Google." : "Google sign-in failed.");
  ...
  return (
    <AuthCard>
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        ...
        {error && (
          <p className="text-xs text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2">
            {error}
          </p>
        )}
      ...
      <button type="button" onClick={handleGoogle} disabled={googleLoading}
        className="flex items-center justify-center gap-3 w-full px-5 py-3 rounded-xl border border-[#e7dcee] dark:border-[#3a2560] bg-white dark:bg-[#1c1030] text-[#2E1A47] dark:text-[#DFC8E7] text-sm font-semibold hover:bg-[#f9f4fa] dark:hover:bg-[#2a1c44] active:scale-[0.98] transition-all shadow-sm disabled:opacity-50">
        {googleLoading ? (
