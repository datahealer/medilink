...
| 6.8 | Settings "Privacy" | 🚫 | Low | No | — | **Hidden for v1** (D3). No policy content yet. |
| 6.9 | `PhoneField` country picker | 🚫 | Low | No | — | **Not required for v1** (D4). Oman-only product; `+968` retained. |
| 6.10 | payment-success bounded auto-poll | ☑ | Low | No | 2–3 h | **Done.** Verify every 3s, max 6 attempts, stops on paid; manual Retry remains. |
...
Lab Reports (list)                 ✅
Lab Trend Chart                    ✅  (real analyte trend, on-demand per analyte)
Prescriptions (read)               ✅
Prescription Reminders             🚫  (hidden for v1)
...
Delete Account                     ✅
Export Data / Privacy              🚫  (hidden for v1; export → Phase 7/AAL2)
Localization (EN/AR)               ✅
