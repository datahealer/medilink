...
BEGIN
  ...
  EXCEPTION
    WHEN unique_violation THEN
      -- Check if THIS patient already owns an active appointment for this slot.
      -- This handles stale state from the old claim flow where book_appointment_atomic
      -- succeeded but the subsequent waitlist update failed, leaving the entry 'offered'
      -- while a pending appointment already exists.
      SELECT id INTO v_appt_id
      FROM public.appointments
      WHERE patient_id = v_entry.patient_id
        AND doctor_id  = (v_slot->>'doctor_id')::UUID
        AND slot_date  = (v_slot->>'slot_date')::DATE
        AND slot_start = (v_slot->>'slot_start')::TIME
        AND status IN ('pending', 'confirmed', 'checked_in');

      IF FOUND THEN
        -- Patient already owns this appointment — reconcile the waitlist entry
        UPDATE public.waitlist_entries
        SET status = 'booked'
        WHERE id = p_entry_id AND status = 'offered';
        RETURN json_build_object('success', true, 'appointment_id', v_appt_id);
      ELSE
        -- Slot is genuinely held by a different patient
        RETURN json_build_object('success', false, 'error', 'SLOT_TAKEN_BY_ANOTHER');
      END IF;
